...
const Profile = () => {
    ...
    const [myRegistrations, setMyRegistrations] = useState<StaffService[]>([]);
    ...
    useEffect(() => {
        ...
        const fetchMyRegs = async () => {
            ...
            if (data) setMyRegistrations(data as StaffService[]);
        };
    ...
    const handleServiceClick = (service: Service) => {
        ...
        const hasDegree = profile?.degree_img && (profile.degree_img as string[]).length > 0;
    ...
    const handleUploadDoc = async (file: File, type: 'cccd_front' | 'cccd_back' | 'degree') => {
        try {
            ...
            if (type === 'degree') {
                const currentDegrees = (profile?.degree_img as string[]) || [];
    ...
    const executeDelete = async () => {
        ...
        try {
            ...
            if (deleteConfirm.type === 'degree') {
                const newDegrees = ((profile?.degree_img as string[]) || []).filter(img => img !== deleteConfirm.imgUrl);
                            ...
                            <div className="bg-white rounded-2xl shadow-sm border p-6 mb-8 border-l-4 border-l-orange-500">
                                    ...
                                    <div className="border p-4 rounded-xl text-center bg-gray-50 hover:shadow-md transition relative group">
                                        ...
                                        <div className="h-40 bg-white rounded-lg border border-gray-200 mb-3 flex items-center justify-center overflow-hidden relative">
                                            {profile?.cccd_front_img ? (
                                                <><img src={profile.cccd_front_img as string} className="w-full h-full object-cover" /><button onClick={() => openDeleteConfirm('cccd_front', profile.cccd_front_img as string)} className="absolute top-2 right-2 bg-red-600 text-white p-1.5 rounded-full shadow-md hover:bg-red-700 transition"><Trash2 size={16} /></button></>
                                    ...
                                    <div className="border p-4 rounded-xl text-center bg-gray-50 hover:shadow-md transition relative group">
                                        ...
                                        <div className="h-40 bg-white rounded-lg border border-gray-200 mb-3 flex items-center justify-center overflow-hidden relative">
                                            {profile?.cccd_back_img ? (
                                                <><img src={profile.cccd_back_img as string} className="w-full h-full object-cover" /><button onClick={() => openDeleteConfirm('cccd_back', profile.cccd_back_img as string)} className="absolute top-2 right-2 bg-red-600 text-white p-1.5 rounded-full shadow-md hover:bg-red-700 transition"><Trash2 size={16} /></button></>
                                            ) : <span className="text-gray-300 text-xs">Chưa có ảnh</span>}
                                        </div>
                                ...
                                <div className="border-t pt-6">
                                    ...
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                        {profile?.degree_img && (profile.degree_img as string[]).map((imgUrl, idx) => (
                                        ...
                                        {(!profile?.degree_img || (profile.degree_img as string[]).length === 0) && <div className="col-span-2 md:col-span-4 text-center py-6 border-2 border-dashed border-gray-200 rounded-lg bg-gray-50"><p className="text-sm text-gray-400">Chưa có bằng cấp nào được tải lên.</p></div>}
                                    </div>
                                </div>
                            </div>
                        </>
